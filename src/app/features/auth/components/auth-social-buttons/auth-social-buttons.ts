...
const BUTTON_CLASSES =
  'inline-flex h-11 w-full items-center justify-center gap-2.5 rounded-panel border border-line bg-surface text-sm font-medium text-ink shadow-soft transition-all duration-200 hover:border-accent/40 hover:bg-surface-muted hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/50 active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-60';
...
export class AuthSocialButtons {
  ...
  protected onGoogle(): void {
    ...
    this.googleIdentity.requestIdToken().subscribe({
      next: (credential) => {
        this.authService.loginWithGoogle(credential, this.rememberMe()).subscribe({
          next: (user) => {
          ...
          error: (error: unknown) => {
            this.googleBusy.set(false);
            this.failed.emit(errorCode(error));
          },
        });
      },
      error: (error: unknown) => {
